...
from model_termsandconditions import model_termsandconditions
...
@app.blob_trigger(arg_name="myblob", path="data/termsandconditions/{name}",
                               connection="STORAGE_CONNECTION_STRING")
@app.cosmos_db_output(arg_name="outputDocument",
                      database_name="ContosoDB",
                      container_name="TermsAndConditions",
                      connection="COSMOS_CONNECTION_STRING")
def ProcessTermsAndConditions(myblob: func.InputStream, outputDocument: func.Out[func.Document]):
    """
    Triggered when a new blob is added to the 'termsandconditions' container.
    Processes the blob and saves the results to Cosmos DB if it is a JSON file.
    ...
    outputDocument (func.Out[func.Document]): The output document for Cosmos DB.
    """
    container_name = 'data/termsandconditions'
    blob_service_client = get_blob_service_client()
    ...
    if file_name.endswith('.json'):
        ...
        # Model the termsandconditions data
        document = model_termsandconditions(data)
